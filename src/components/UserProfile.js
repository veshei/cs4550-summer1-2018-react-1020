...
export default class UserProfile extends React.Component {
    ...
    saveProfile() {
        ...
        if (this.state.firstName.length <= 0) {
            console.log("GotHere");
            this.setState({
                activeWarning: 'Please enter your first name',
                showWarning: true
            })
        }
        else if (this.state.lastName.length <= 0) {
            this.setState({
                activeWarning: 'Please enter your last name',
                showWarning: true
            })
        }
        else if (this.state.dateOfBirth.length <= 0) {
            this.setState({
                activeWarning: 'Please enter your date of birth',
                showWarning: true
            })
        }
        else if (new Date(this.state.dateOfBirth) > Date.now()) {
            this.setState({
                activeWarning: 'Your birthday cannot be in the future',
                showWarning: true
            })
        }
        else {
